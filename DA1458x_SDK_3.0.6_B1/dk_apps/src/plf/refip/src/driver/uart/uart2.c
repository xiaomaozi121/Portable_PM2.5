...
static void user_uart2_rec_data_avail_isr(void)
{
	  #if (BLE_SAMPLE128)
		// Read the received in the FIFO
		uart_buf[uart_bufcnt]= uart2_rxdata_getf();
		if(uart_buf[0]==0x55)
		{
				uart_bufcnt++;
				if(uart_bufcnt==5 )
				{		
						...
						if(uart_buf[4]==0xAA)
						{
							...
							{
								app_adv_start();
								uart2_write("adv_Start",strlen("adv_Start"),NULL);
							}
							else if(uart_buf[1]==0x02)
							{
								app_adv_stop();
								uart2_write("adv_Stop",strlen("adv_Stop"),NULL);
							}
							else if(uart_buf[1]==0x04)
							{
								flag_PM25=1;
								periph_init();
								...
								PM2_5_val=uart_buf[2]*256+uart_buf[3];
								...
								{
										PM2_5[6]=(PM2_5_val/100)+0x30;
								}
								if((PM2_5_val%100/10)==0 && PM2_5[6]==' ')
								{
										PM2_5[7]=' ';
								}
								else
								{
										PM2_5[7]=(PM2_5_val%100/10)+0x30;
								}
								PM2_5[8]=(PM2_5_val%10)+0x30;
								if(flag_Connect)
								{
