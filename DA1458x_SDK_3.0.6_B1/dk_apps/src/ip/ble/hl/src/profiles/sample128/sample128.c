...
void sample128_send_val(my_newer1_addChar val)
{
   ...
   //ind->val = val;
   memcpy(&ind->val,val,sizeof(my_newer1_addChar));
   // Send the message
   ke_msg_send(ind);
}
...
{
    // Allocate the alert value change indication
   struct sample128_C2_val_ind *ind = KE_MSG_ALLOC(SAMPLE128_C2_VAL_IND,
                                              sample128_env.con_info.appid, TASK_SAMPLE128,
                                              sample128_C2_val_ind);
   // Fill in the parameter structure
   ind->conhdl = gapc_get_conhdl(sample128_env.con_info.conidx);
   //ind->val = val;
   memcpy(&ind->val,val,sizeof(my_newer2_addChar));
   // Send the message
   ke_msg_send(ind);
}
