                                       ...
                                       ke_task_id_t const src_id)
{
     ...
     *---------------------------------------------------*/
        ...
        status = attmdb_add_service(&(sample128_env.sample128_shdl), TASK_SAMPLE128,
                                             nb_att_16, nb_att_32, nb_att_128, 56+characristic1_length+characristic2_length);  //58 ÎÒÓÃ8bytesµÄÊý×é´úÌæÁËÌØÐÔÖµ2
																						 //total attributte size = 36, 16 (svc)  + 19 (desc_char) + 1 (att)

        if (status == ATT_ERR_NO_ERROR)
        {
            //add svc attribute
						//sample128_envÊÇ¶¨ÒåµÄ»·¾³±äÁ¿
						//sample128_env.sample128_shdl »·¾³±äÁ¿½á¹¹ÌåµÄÆðÊ¼²Ù×
						/*
             ...
             */        
            status = attmdb_att_set_value(sample128_env.sample128_shdl, ATT_UUID_128_LEN, 
																							(uint8_t *)sample128_svc.uuid);
            
            
///////char 1     
            //add char attribute
            status = attmdb_add_attribute(sample128_env.sample128_shdl, ATT_UUID_128_LEN + 3, 
																							//Data size = 19 (ATT_UUID_128_LEN + 3)
            ...
            //add val attribute
            status = attmdb_add_attribute(sample128_env.sample128_shdl, sizeof(my_newer1_addChar), 
																							//Data size = 1
...
//////char 2
            ...
            //add val attribute
            status = attmdb_add_attribute(sample128_env.sample128_shdl, sizeof(my_newer2_addChar), //Data size = 1
                                               ATT_UUID_128_LEN, (uint8_t*)&sample128_2_val.uuid, PERM(RD, ENABLE) | PERM(WR, ENABLE) |PERM(NTF, ENABLE),
                                               &(val_hdl));
                                    ...
                                    ke_task_id_t const src_id)
{
    ...
    else
    {
        ...
        attmdb_att_set_value(sample128_env.sample128_shdl + SAMPLE128_1_IDX_VAL,
                             sizeof(my_newer1_addChar), (uint8_t *)&param->sample128_1_val);
        
        attmdb_att_set_value(sample128_env.sample128_shdl + SAMPLE128_2_IDX_VAL,
                             sizeof(my_newer2_addChar), (uint8_t *)&param->sample128_2_val);
                                    ...
                                    ke_task_id_t const src_id)
{
    ...
    if(param->conhdl == gapc_get_conhdl(sample128_env.con_info.conidx))
    {
        // Update value in DB
        attmdb_att_set_value(sample128_env.sample128_shdl + SAMPLE128_2_IDX_VAL,
                             sizeof(my_newer2_addChar), (uint8_t *)&param->val);
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    if (KE_IDX_GET(src_id) == sample128_env.con_info.conidx)
    {
        ...
        if (char_code == SAMPLE128_1_CHAR)
        {
            ...
            //Save value in DB
            attmdb_att_set_value(param->handle, sizeof(my_newer1_addChar), (uint8_t *)&param->value[0]);
            
            if(param->last)
            {
                sample128_C2_send_val((uint8_t *)&param->value[0]);
            }

            status = PRF_ERR_OK;
               
        }
				if (char_code == SAMPLE128_2_CHAR)
        {
