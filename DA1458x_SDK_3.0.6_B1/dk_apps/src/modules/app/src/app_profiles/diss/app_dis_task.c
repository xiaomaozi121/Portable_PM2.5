                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    if (ke_state_get(dest_id) == APP_DB_INIT)
    {
        ...
        {
						if(APP_DIS_FEATURES|DIS_MANUFACTURER_NAME_CHAR_SUP)
				    {
								// Go to the idle state
								// Set Manufacturer Name value in the DB
								len = strlen(APP_DIS_MANUFACTURER_NAME);
                ...
                req_name->val_len       = len;
                memcpy(&req_name->val[0], APP_DIS_MANUFACTURER_NAME, len);
            ...
            // Set Model Number String value in the DB
						if(APP_DIS_FEATURES|DIS_MODEL_NB_STR_CHAR_SUP)
            {
								len = strlen(APP_DIS_MODEL_NB_STR);
                ...
                req_mod->val_len       = len;
                memcpy(&req_mod->val[0], APP_DIS_MODEL_NB_STR, len);
            ...
            // Set System ID value in the DB
						if(APP_DIS_FEATURES|DIS_SYSTEM_ID_CHAR_SUP)
            {
								len = strlen(APP_DIS_SYSTEM_ID);
                ...
                req_id->val_len       = len;
                memcpy(&req_id->val[0], APP_DIS_SYSTEM_ID, len);
                ...
                ke_msg_send(req_id);
						...
						// Set Manufacturer Name value in the DB
						if(APP_DIS_FEATURES|DIS_HARD_REV_STR_CHAR_SUP)						
						{
								len = strlen(APP_DIS_HW_REV);              
                struct diss_set_char_val_req *req_hw = KE_MSG_ALLOC_DYN(DISS_SET_CHAR_VAL_REQ,
                                                                          TASK_DISS, TASK_APP,
                                                                          diss_set_char_val_req,
                                                                          len);

                // Fill in the parameter structure
                req_hw->char_code     = DIS_HARD_REV_STR_CHAR;
                req_hw->val_len       = len;
                memcpy(&req_hw->val[0], APP_DIS_HW_REV, len);
            ...
            // Set the software version in the DB. This is the reference design se version
						if(APP_DIS_FEATURES|DIS_SW_REV_STR_CHAR_SUP)	
            {
            ...
            // Set the firmware version in the DB. This is the common code sw version
						if(APP_DIS_FEATURES|DIS_FIRM_REV_STR_CHAR_SUP)	
            {
            ...
            // Set the PNP ID in the DB
						if(APP_DIS_FEATURES|DIS_PNP_ID_CHAR_SUP)	
            {
                uint16 ids[3];
                struct diss_set_char_val_req *req_id = KE_MSG_ALLOC_DYN(DISS_SET_CHAR_VAL_REQ,
                                                                        TASK_DISS, TASK_APP,
